...
void setup()
{
  ...
  buzzer.init(BUZZER_PIN);  // initialize buzzer system
  buzzer.mute(); // mute sounds for now
  buzzer.beep(200,600);
  oledDisplay.init();  // initialize display
  input.init();   // initialize input system
  changeAppMode(0);// start main menu display
}
